...
describe 'mutation userAddWatchlistMovie', type: :request do
  ...
  context 'when user is authenticated' do
    it 'returns movie data' do
      graphql_post(
        query: user_add_watchlist_movie_mutation,
        variables: variables,
        headers: { 'Authorization': "Bearer #{token}" }
      )

      expect(response).to match_schema(User::AddWatchlistMovieSchema::Success)
